...
{
    if (argc != 3) {
        fprintf(stderr, "usage: input.pcm output.pcm\n");
        return 1;
    }
    ...
    while (!feof(fin)) {
        ...
        // (Oversampling)
        // window + 2 = {0,0,0,0,0,0,x,x} window = {0,0,0,0,0,0,0,0}
        // window = {0,0,0,0,0,0,0,0}  
        // samples = {1,2}
        // window = {0,0,0,0,0,0,1,2}  
        ...
        // Forward FFT
        fftw_execute(forward);  // infft -> outfft
        ...
        // printf("\n");
        // Pitch Adjustment.
        pitchadjust(outfft, infft); // outfft -> infft (just use same var to pitch adjust)
        // for (i = 0; i < N; i++) {
        ...
        fftw_execute(reverse); // infft -> outfft
        for (i = 0; i < N; i++) {
            outfft[i] /= (double)N;
        }
